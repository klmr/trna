...
trnaPlotDeCountMatrix <- function () {
    maxval <- lapply(trnaDeCounts, partial(max, na.rm = TRUE))
    ...
    w <- nrow(trnaDeCounts[[1]])
    par(mfrow = c(w, w), mar = rep(0, 4), oma = c(1, 1, 4, 10))
    for (i in seq(w)) {
        for (j in seq(w)) {
            tissue <- if (i < j) 'liver' else 'brain'
            value <- trnaDeCounts[[tissue]][i, j]
            plot.new()
            rect(par('usr')[1], par('usr')[3], par('usr')[2], par('usr')[4],
                 col = colors[[tissue]][value + 1], border = NA)
            if (i == j)
                text(0.5, 0.5, labels = colnames(trnaDeCounts[[tissue]])[i], cex = 2)
            else
                text(0.5, 0.5, labels = value, cex = 1.5)
        }
    }

    title('Number of differentially expressed genes', cex.main = 2, outer = TRUE)
